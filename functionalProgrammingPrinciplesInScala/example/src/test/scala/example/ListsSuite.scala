...
class ListsSuite {
  ...
  @Test def testMax1: Unit = {
    assert(max(List(-1, 0, 4, -2, 1)) == 4)
  }

  @Test def testMax2: Unit = {
    try {
      max(List())
      Assert.fail("No exception thrown with empty list")
    } catch {
      case e: NoSuchElementException => ()
    }
  }

  @Test def testMax3: Unit = {
    assert(max(List(2, 3, 2, 1, 3, 2, 0)) == 3)
  }

  @Test def testMax4: Unit = {
    assert(max(List(0)) == 0)
  }

  @Test def testSum1: Unit = {
    assert(sum(List(-3, 3, 2, 1, 0)) == 3)
  }
